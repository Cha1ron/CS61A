...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, pw):
        nonlocal balance, password, withdraw_attempts
        if withdraw_attempts == 3:
            return "Too many incorrect attempts. Attempts: " + str(attempt_list)
        elif pw != password:
            withdraw_attempts += 1
            attempt_list.append(pw)
...
def repeated(t, k):
    """Return the first value in iterator T that appears K times(连续的) in a row. Iterate through the items such that
